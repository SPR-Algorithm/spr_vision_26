...
PowerRune::PowerRune(std::vector<FanBlade> &ts, const cv::Point2f center,
  ...
  // 没有新亮起来的fanblade
           ...
           ts.size() == last_powerrune.value().light_num) {
  ...
  // 有新亮起来的fanblade
           ...
           light_num == last_powerrune.value().light_num + 1) {
  ...
  /// 状态分类: 依据 target 扇叶的类别推断大小符
  if (ts[0].cls == 2)
    type = BIG;
  else if (ts[0].cls == 1)
    type = SMALL;
  else
    type = SMALL; // 未激活/未知默认按小符处理
  ...
  const std::vector<double> target_angles = {
      0, 2.0 * CV_PI / 5.0, 4.0 * CV_PI / 5.0, 6.0 * CV_PI / 5.0,
      8.0 * CV_PI / 5.0};
