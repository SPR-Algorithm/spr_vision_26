...
YOLO11_BUFF::YOLO11_BUFF(const std::string & config)
{
  ...
  // RuneDetectionModel 固定 3 类别, 关键点数 = (通道数 - 类别数) / kpt_dim
  num_classes_ = 3;
  num_keypoints_ = (output_channels_ - num_classes_) / kpt_dim_;
  if (num_keypoints_ * kpt_dim_ + num_classes_ != output_channels_) {
    throw std::runtime_error("[YOLO11_BUFF] 输出通道数与 3类别+5关键点 不匹配");
  }
...
{
  scale = std::min(
    static_cast<float>(input_width_) / src.cols, static_cast<float>(input_height_) / src.rows);
  ...
  float scale, int pad_w, int pad_h, int orig_w, int orig_h)
{
  ...
  objects.reserve(64);
  const float * output_data = infer_request.get_output_tensor().data<float>();

  // 输出布局 [1, C, A]: 通道 c 第 a 个 anchor 的索引为 c*num_anchors_ + a
  for (int i = 0; i < num_anchors_; ++i) {
    int best_cls = -1;
    float max_conf = 0.0f;
    for (int c = 0; c < num_classes_; ++c) {
      const float score = output_data[c * num_anchors_ + i];
      if (score > max_conf) {
        max_conf = score;
        best_cls = c;
      }
    ...
    if (best_cls < 0 || max_conf < conf_threshold_) continue;
    ...
    for (int k = 0; k < num_keypoints_; ++k) {
      ...
      float x = (output_data[(base + 0) * num_anchors_ + i] - pad_w) / scale;
      float y = (output_data[(base + 1) * num_anchors_ + i] - pad_h) / scale;
      const float k_conf = output_data[(base + 2) * num_anchors_ + i];
      ...
      obj.kpt.emplace_back(x, y);
      obj.kpt_conf.push_back(k_conf);
      center += obj.kpt.back();
      if (k_conf >= kpt_conf_threshold_) ++valid_kpts;
    }

    if (has_invalid_kpt || valid_kpts < std::min(min_valid_kpts_, num_keypoints_)) continue;

    obj.center = center * (1.0f / num_keypoints_);
...
std::vector<YOLO11_BUFF::Object> YOLO11_BUFF::nms(std::vector<Object> & objects)
{
  ...
  for (size_t i = 0; i < objects.size(); ++i) {
    ...
    for (size_t j = i + 1; j < objects.size(); ++j) {
      ...
      if (dx * dx + dy * dy < dist_thresh_sq) suppressed[j] = true;
    }
  }
  return result;
}
...
std::vector<YOLO11_BUFF::Object> YOLO11_BUFF::get_multicandidateboxes(cv::Mat & image)
{
  ...
  int pad_w = 0, pad_h = 0;
  cv::Mat preprocessed;
  letterbox(image, preprocessed, scale, pad_w, pad_h);
...
std::vector<YOLO11_BUFF::Object> YOLO11_BUFF::get_onecandidatebox(cv::Mat & image)
{
  ...
  if (objects.empty()) return std::vector<Object>();
...
void YOLO11_BUFF::draw(cv::Mat & image, const std::vector<Object> & objects)
{
  for (const auto & obj : objects) {
    ...
    for (size_t k = 0; k < obj.kpt.size(); ++k) {
      cv::Scalar color =
        obj.kpt_conf[k] >= kpt_conf_threshold_ ? cv::Scalar(0, 255, 0) : cv::Scalar(0, 0, 255);
      cv::circle(image, obj.kpt[k], 3, color, -1);
      cv::circle(image, obj.kpt[k], 3, cv::Scalar(255, 255, 255), 1);
      cv::putText(
        image, std::to_string(k + 1), obj.kpt[k] + cv::Point2f(5, -5), cv::FONT_HERSHEY_SIMPLEX,
    ...
    cv::rectangle(
      ...
      cv::Point(obj.center.x + text_size.width + 2, obj.center.y + 2), cv::Scalar(0, 0, 0), -1);
    cv::putText(
      image, label, cv::Point(obj.center.x, obj.center.y - 2), cv::FONT_HERSHEY_SIMPLEX, 0.5,
