...
void capture_loop(const std::string &config_path, const std::string &can,
  ...
  int count = 0;
  const cv::Size pattern_size(8, 11);
  while (true) {
    ...
    tools::draw_text(img_with_ypr, fmt::format("Z {:.2f}", zyx[0]), {40, 40},
                     {0, 0, 255});
    tools::draw_text(img_with_ypr, fmt::format("Y {:.2f}", zyx[1]), {40, 80},
                     {0, 0, 255});
    tools::draw_text(img_with_ypr, fmt::format("X {:.2f}", zyx[2]), {40, 120},
                     {0, 0, 255});
    ...
    cv::drawChessboardCorners(img_with_ypr, pattern_size, centers_2d,
                              success);                   // 显示识别结果
...
int main(int argc, char *argv[]) {
  ...
  tools::logger()->info("默认标定板尺寸为8列11行，方格边长25mm");
  // 主循环，保存图片和对应四元数
