...
std::vector<cv::Point3f> centers_3d(const cv::Size &pattern_size,
...
void load(const std::string &input_folder, const std::string &config_path,
  ...
  for (int i = 1; true; i++) {
    ...
    Eigen::Vector3d ypr =
        tools::eulers(R_gimbal2world, 2, 1, 0) * 57.3; // degree

    // 在图片上显示云台的欧拉角，用来检验R_gimbal2imubody是否正确
    auto drawing = img.clone();
    tools::draw_text(drawing, fmt::format("yaw   {:.2f}", ypr[0]), {40, 40},
                     {0, 0, 255});
    tools::draw_text(drawing, fmt::format("pitch {:.2f}", ypr[1]), {40, 80},
                     {0, 0, 255});
    tools::draw_text(drawing, fmt::format("roll  {:.2f}", ypr[2]), {40, 120},
                     {0, 0, 255});
    ...
    auto success = cv::findChessboardCorners(
        ...
        cv::CALIB_CB_ADAPTIVE_THRESH +
            cv::CALIB_CB_NORMALIZE_IMAGE); // 默认是棋盘格图案
    ...
    cv::solvePnP(centers_3d_, centers_2d, camera_matrix, distort_coeffs, rvec,
...
void print_yaml(const std::vector<double> &R_gimbal2imubody_data,
  ...
  YAML::Emitter result;
  std::vector<double> R_camera2gimbal_data(R_camera2gimbal.begin<double>(),
                                           R_camera2gimbal.end<double>());
  std::vector<double> t_camera2gimbal_data(t_camera2gimbal.begin<double>(),
  ...
  result << YAML::Comment(fmt::format(
      "相机同理想情况的偏角: yaw{:.2f} pitch{:.2f} roll{:.2f} degree", ypr[0],
      ypr[1], ypr[2]));
...
int main(int argc, char *argv[]) {
  ...
  load(input_folder, config_path, R_gimbal2imubody_data, R_gimbal2world_list,
       t_gimbal2world_list, rvecs, tvecs);
